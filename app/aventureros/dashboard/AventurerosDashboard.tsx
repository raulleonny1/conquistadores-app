...
import { LOGO_AVENTUREROS } from "@/src/constants/programLogos";
...
export default function AventurerosDashboard() {
  const params = useSearchParams();
  const router = useRouter();
  const pin = params.get("pin") ?? "";
  const clubSlug = params.get("club") ?? "";

  const [perfil, setPerfil] = useState<AventureroPerfil | null>(null);
  ...
  useEffect(() => {
    if (!pin || !clubSlug) {
      setError("Falta el PIN o el código del club. Entra desde el login de Aventureros.");
      setLoading(false);
      return;
    }
    ...
    (async () => {
      try {
        ...
        if (!club.programas.includes("aventureros")) {
          if (!cancelado) setError("Este club no tiene habilitado el programa Aventureros.");
          return;
        }
  ...
  useEffect(() => {
    if (!pin) return;
    const ref = doc(db, COLECCION_POR_PROGRAMA.aventureros, pin);
  ...
  if (error || !perfil) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-amber-950 px-6 text-center text-white">
        <p className="text-lg font-semibold text-amber-200">{error || "Sin datos"}</p>
        <Link
          href="/login/aventureros"
          className="mt-6 rounded-xl bg-amber-500 px-6 py-3 font-bold text-amber-950 hover:bg-amber-400"
        >
          Ir al login
        </Link>
      </div>
  ...
  const progreso = progresoInsigniasClase(perfil.clase, perfil.insignias);
  const siguienteClase = getSiguienteClaseAventurero(perfil.clase);
  const progresoPrograma = progresoProgramaAventurero(perfil.clase, perfil.insignias);
  const totalPuntos = sumarPuntos(puntos);
  const categorias = getCategoriasConPuntos(puntos, etiquetas);

  return (
    <div className="min-h-screen bg-linear-to-b from-amber-950 via-slate-950 to-slate-950 text-white">
      <header className="border-b border-amber-500/20 bg-amber-950/50 px-6 py-5">
        <div className="mx-auto flex max-w-3xl items-center justify-between">
          <div className="flex items-center gap-3">
            <Image src={LOGO_AVENTUREROS} alt="Aventureros" width={40} height={40} className="rounded-lg" />
            <div>
        ...
        <section className="rounded-3xl border border-amber-500/25 bg-amber-500/10 p-6">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              {perfil.club && (
                <p className="mt-1 text-sm text-amber-200/70">Club: {perfil.club}</p>
              )}
            </div>
          </div>
        </section>
            ...
            <ul className="grid gap-2 sm:grid-cols-2">
              {categorias.map((c) => (
                ...
                >
                  ...
                  <span className="font-bold text-amber-400">{c.valor}</span>
                </li>
              ))}
            </ul>
          )}
